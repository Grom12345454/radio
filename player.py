...
class RadioPlayer:
    ...
    def set_callbacks(self, on_state_changed=None, on_error=None):
    ...
    def play(self, station_name, url):
        ...
        try:
            ...
            media.add_option(f":network-caching={NETWORK_CACHING}")
            ...
            if result == -1:
                ...
                if self.on_error:
                    self.on_error("VLC не смог запустить поток.")
            ...
            if self.on_error:
                self.on_error(f"Ошибка запуска: {error}")
